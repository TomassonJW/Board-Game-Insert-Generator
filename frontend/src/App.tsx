...
export default function App() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      setMessage('Le dossier de préparation technique a été téléchargé. Il ne crée ni bac, ni scène Fusion, ni pièce imprimable.')
    } catch (reason) {
      setError(formatError(reason))
    } finally {
      setBusy(false)
    ...
    <main className="app-shell">
      ...
      <section className="studio-workspace">
        ...
        <section className="studio-editor" aria-labelledby="current-step-title">
          ...
          {activeStep === 'prepare' && <section className="editor-card prepare-card"><div className="card-intro"><h3>{selectedVariant ? 'Ton organisation est choisie' : 'Choisis d’abord une idée'}</h3><p>{selectedVariant ? `« ${proposalExplanation(selectedVariant.policy_id).title} » est retenue. Tu peux l’enregistrer comme décision, mais elle ne contient pas encore de vrai bac.` : 'Reviens à l’étape “Les idées” pour sélectionner une organisation.'}</p></div><div className="manufacturing-path" aria-label="Étapes de fabrication"><div className="active"><span>1</span><strong>À explorer</strong><small>Organisation numérique choisie</small></div><div><span>2</span><strong>À vérifier dans Fusion</strong><small>Après la génération de vrais bacs</small></div><div><span>3</span><strong>À imprimer et mesurer</strong><small>Validation physique nécessaire</small></div></div>{selectedVariant && <div className="prepare-actions"><button className="button secondary" disabled={busy} onClick={lockSelectedProposal}>Conserver cette organisation</button><button className="button primary" onClick={() => downloadJson(`${safeFileName(draft.project_name)}-projet.json`, draft)}>Sauvegarder le projet</button></div>}<details className="technical-download"><summary>Mode expert : télécharger le dossier de préparation</summary><p>Ce téléchargement contient une décision et un fichier technique pour le pipeline existant. À ce stade, il ne génère aucun bac, aucune scène Fusion et aucune pièce imprimable.</p><button className="button quiet" disabled={!selectedVariant || busy} onClick={() => void handleExport()}>Télécharger le dossier technique</button></details></section>}
        </section>
      </section>
    </main>
  )
...
function ManufacturingStatus({ selectedVariant }: { selectedVariant?: Variant }) {
  return <div className="manufacturing-status"><span className="status-dot" /><div><strong>{selectedVariant ? 'À explorer' : 'Projet en préparation'}</strong><small>{selectedVariant ? 'Organisation choisie, aucun bac imprimable pour le moment.' : 'Ajoute les mesures pour commencer.'}</small></div></div>
}
