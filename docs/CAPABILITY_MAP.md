...
| C-GRID-3D | 3D volumetric grid | Volumetric layout | `implemented-cad-ir` | M7 Volumetric planner | P8-M001 implemente le socle declaratif; P10-M008 utilise un placement greedy abstrait; P11-M001 est `fusion-validated` pour la consommation compacte des placements, sans validation physique |
...
| C-FUSION-COMPACT | Fusion compact view | CAD generation pipeline | `fusion-validated` | M3 CAD pipeline / M7 Volumetric planner | Blanks rectangulaires valides; vue compacte grille P11-M001 validee manuellement dans Fusion, print-validated: false |
| C-FUSION-EXPLODED | Fusion exploded view | CAD generation pipeline | `implemented-fusion` | M5 CAD inspection views | P7-M001 code une vue eclatee basique corrigee par composants uniques et occurrences compactes/eclatees liees; Part Design est detecte comme incompatible; le renommage direct de `Occurrence.name` est evite; smoke test P7-M001V4 requis en document Assembly-compatible; print-validated: false |
