...
| C-ACCESS | Accessibility and removal order | Human validation gates | `implemented-fusion` | M8 Ergonomic planner | Removal order abstrait P8-M002; P13-ASSET-M005 implemente une encoche rectangulaire top-open par compartiment supporte, validation Fusion humaine P13-ASSET-M005V requise avant statut fusion-validated; validation ergonomique/impression reelle non revendiquee |
...
| C-FILLETS | Fillets and finger notches | CAD generation pipeline | `fusion-validated` | M5 CAD ergonomic features | Encoche rectangulaire top-open P6 validee Fusion, print-validated: false ; P13-ASSET-M005 reutilise cette coupe pour les access notches asset-first mais attend P13-ASSET-M005V ; courbes/fillets bloques |
...
- Gate active : `P13-ASSET-M005V`; aucune validation d'impression.

## P13-ASSET-M005 capability update

- C-ASSET : `implemented-fusion` pour metadata `asset_access_notch` par compartiment supporte ; Fusion validation humaine requise avant `fusion-validated-v0`.
- C-ACCESS : `implemented-fusion` pour la premiere aide de prise asset-first V0, rectangulaire top-open et front-wall-only.
- C-FILLETS : reuse de la coupe rectangulaire top-open existante, sans courbes, demi-lunes reelles, scoops, fillets ou conges.
- C-FUSION-UI : reporting `asset_access_*` et smoke quick_asset_box prepares ; `P13-ASSET-M005V` est la gate active.
