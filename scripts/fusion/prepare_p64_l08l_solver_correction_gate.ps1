...
$preflight = Join-Path $root "scripts\fusion\p64_l08l_preflight.py"
...
$fixturePath = Join-Path $projectDirectory "p64-l08l-public-28x30.bgig.json"
$summaryPath = Join-Path $projectDirectory "p64-l08l-preflight-summary.json"
...
$workspaceTemp = Join-Path $root ".codex-work\p64-l08l-fusion-gate"
$tempFixture = Join-Path $workspaceTemp "p64-l08l-public-28x30.bgig.json"
$tempSummary = Join-Path $workspaceTemp "p64-l08l-preflight-summary.json"

if (-not (Test-Path -LiteralPath $preflight -PathType Leaf)) {
    throw "P64-L08L preflight script missing: $preflight"
}
...
if ($expectedVersion -ne "0.1.62") {
    throw "P64-L08L package version mismatch: expected 0.1.62, got $expectedVersion."
}

Write-Output "BGIG P64-L08L SCIP product Fusion gate preparation"
Write-Output "Repo root: $root"
Write-Output "Commit: $commit"
Write-Output "Package version: $expectedVersion"
Write-Output "Target: $target"
Write-Output "Public repeated-fill fixture: $fixturePath"
...
try {
    ...
    foreach ($pattern in @(
        ...
        "test_p64_l08l_fusion_preflight.py",
...
if ($DryRun) {
    ...
    Write-Output "Dry run: would install the public 28x30 fixture and select Auto intelligent + Approfondi."
    ...
    try {
        ...
        if ($artifact.artifact_digest -ne "05d4566e93efef2b6606b0d1807abaaf29bc460c37accee31da20ae2a6462065") {
            throw "Installed P64-L08L SCIP artifact digest mismatch."
        }
        ...
        if ($archiveHash -ne "0a718ea5884d6326d66777db0ab853a31fa981e6392b89f184342fde27d465c6") {
            throw "Installed P64-L08L SCIP archive digest mismatch."
        }
        ...
        foreach ($marker in @(
            ...
            if (-not $installedMinimalSolver.Contains($marker)) {
                throw "Installed P64-L08L minimal solver marker missing: $marker"
            }
        }
        ...
        foreach ($marker in @("hybrid_anchor_and_fill", "solution_limit", "wall_seconds=120.0")) {
            if (-not $installedProductSolver.Contains($marker)) {
                throw "Installed P64-L08L SCIP product marker missing: $marker"
            }
        }
        ...
        if (Test-Path -LiteralPath $fixturePath -PathType Leaf) {
            ...
            if ($existingHash -ne $newHash) {
                $backup = Join-Path $projectDirectory "p64-l08l-public-28x30.before-$($existingHash.Substring(0, 12)).bgig.json"
                if (-not (Test-Path -LiteralPath $backup -PathType Leaf)) {
                    Copy-Item -LiteralPath $fixturePath -Destination $backup -Force
                    Write-Output "Existing L08L fixture preserved: $backup"
                }
            }
        }
        ...
        if (Test-Path -LiteralPath $documentStatePath -PathType Leaf) {
            ...
            $stateBackup = Join-Path $projectDirectory "bgig_document_state_v1.before-p64-l08l-$($stateHash.Substring(0, 12)).json"
        ...
        if ($installedCommit -ne $commit) {
            throw "Installed P64-L08L commit marker mismatch."
        }
        if (-not (Test-Path -LiteralPath $fixturePath -PathType Leaf)) {
            throw "P64-L08L public fixture was not installed."
        }
    }
    catch [UnauthorizedAccessException] {
        Write-Error "P64-L08L local Fusion handoff write blocked: $($_.Exception.Message)"
        exit 21
    }
...
Write-Output ""
Write-Output "P64-L08L Fusion actions remaining:"
Write-Output "1. Recharger l add-in $expectedVersion puis ouvrir la palette BGIG."
Write-Output "2. Verifier que le projet p64-l08l-public-28x30 est charge avec Auto intelligent + Approfondi."
Write-Output "3. Lancer Calculer l agencement une seule fois et attendre au plus 130 secondes."
...
Write-Output "Prepared P64-L08L SCIP product gate: $(-not $DryRun)"