...
class FusionPaletteDomTests(unittest.TestCase):
    ...
    def test_truthful_finalization_colors_and_reactive_budgets_are_explicit(self) -> None:
        for marker in (
            ...
            "function finalizationDiagnostics(response)",
            "bgig.finalization_stop_diagnostics.v1",
            "proof_of_impossibility===true",
            "reported==='proven_impossible'&&!proven?'strategy_exhausted':reported",
            "Le résultat reste inconnu, pas impossible.",
            "Phase : ${phase}. Recherche : ${budget} sur ${cap} max.",
            "Terminaison après recherche : ${formatCalculationMs(termination)}.",
            "Temps mural total : ${total}.",
            "Budget de recherche ${esc(formatOperationElapsed(budget))} atteint · terminaison",
            "Preuve d’impossibilité : ${proven?'oui':'non'}",
            "technicalDetail(presentation.technical)",
