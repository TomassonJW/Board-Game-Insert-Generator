...
class AssetModelTests(unittest.TestCase):
    ...
    def test_multi_asset_compartments_support_four_assets_with_internal_walls(self) -> None:
        ...
        self.assertEqual(report["printability_status"], "warning")
        self.assertTrue(report["printability_export_allowed"])
        self.assertEqual(report["issue_counts"]["blocker"], 0)
        self.assertGreaterEqual(report["issue_counts"]["warning"], 1)
        self.assertTrue(any(issue["code"] == "NO_PHYSICAL_PRINT_VALIDATION" for issue in report["issues"]))
