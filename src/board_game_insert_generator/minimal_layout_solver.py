...
def solve_minimal_layout(
    ...
    *,
    ...
    if (
        ...
        projection_deadline_at_ms = min(
            deadline_at_ms,
            started_at_ms
            + min(
                5_000.0,
                max(1_000.0, solver_deadline_seconds(effort_profile) * 250.0),
            ),
        )
        projected_plan = _solve_minimal_layout_once(
            projected_project,
            ...
            deadline_at_ms=projection_deadline_at_ms,
    ...
    *,
    ...
    if _deadline_has_expired(deadline_at_ms):
        return _failure_plan(
            ...
            stop_reason="global_deadline_reached_without_candidate",
            strategy=strategy,
            budget=budget,
            request=request,
            project_name=str(base_problem.project["project_name"]),
